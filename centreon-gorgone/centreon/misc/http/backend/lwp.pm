...
package centreon::misc::http::backend::lwp;

use strict;
use warnings;
use centreon::misc::http::backend::useragent;
use URI;
use IO::Socket::SSL;
...
sub set_proxy {
    ...
    if (defined($options{request}->{proxypac}) && $options{request}->{proxypac} ne '') {
        if (centreon::misc::misc::mymodule_load(
            logger => $self->{logger}, module => 'HTTP::ProxyPAC',
        ...
        if ($@) {
            $self->{logger}->writeLogError('issue to load proxypac: ' . $@);
            return 1;
        }
...
sub request {
    ...
    if (!defined($self->{ua})) {
        ...
        if (defined($request_options->{cookies_file})) {
            if (centreon::misc::misc::mymodule_load(
                logger => $self->{logger}, module => 'HTTP::Cookies',
    ...
    if (defined($request_options->{credentials}) && defined($request_options->{ntlmv2})) {
        if (centreon::misc::misc::mymodule_load(
            logger => $self->{logger}, module => 'Authen::NTLM',
