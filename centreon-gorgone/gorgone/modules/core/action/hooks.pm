...
sub gently {
    ...
    $stop = 1;
    if (defined($action->{running}) && $action->{running} == 1) {
        $options{logger}->writeLogDebug("[action] Send TERM signal $action->{running}");
...
sub check {
    ...
    foreach my $pid (keys %{$options{dead_childs}}) {
        # Not me
        next if (!defined($action->{pid}) || $action->{pid} != $pid);
