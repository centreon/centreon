...
sub gently {
    ...
    $stop = 1;
    if (defined($engine->{running}) && $engine->{running} == 1) {
        $options{logger}->writeLogDebug("[engine] Send TERM signal $engine->{pid}");
...
sub kill {
    ...
    if ($engine->{running} == 1) {
        $options{logger}->writeLogDebug("[engine] Send KILL signal for pool");
...
sub create_child {
    ...
    $options{logger}->writeLogInfo("[engine] Create module 'engine' process");
    my $child_pid = fork();
    if ($child_pid == 0) {
        $0 = 'gorgone-engine';
    ...
    $options{logger}->writeLogDebug("[engine] PID $child_pid (gorgone-engine)");
