...
package modules::centreon::autodiscovery::hooks;

use warnings;
use strict;
use JSON::XS;
use centreon::script::gorgonecore;
use modules::centreon::autodiscovery::class;
...
sub routing {
    ...
    if (centreon::script::gorgonecore::waiting_ready(ready => \$autodiscovery->{ready}) == 0) {
        centreon::gorgone::common::add_history(
            dbh => $options{dbh},
            code => 30, token => $options{token},
...
sub create_child {
    ...
    if ($child_pid == 0) {
        ...
        my $module = modules::centreon::autodiscovery::class->new(
