...
class JiraProvider extends AbstractProvider
{
    ...
    protected function setDefaultValueExtra()
    {
        ...
        $this->default_data['clones']['mappingTicket'] = [
            ...
            ['Arg' => self::ARG_PROJECT, 'Value' => '{$select.jira_project.id}'],
            ['Arg' => self::ARG_ASSIGNEE, 'Value' => '{$select.jira_assignee.id}'],
            ['Arg' => self::ARG_PRIORITY, 'Value' => '{$select.jira_priority.id}'],
            ['Arg' => self::ARG_ISSUETYPE, 'Value' => '{$select.jira_issuetype.id}']
        ];
    }
    ...
    protected function getConfigContainer1Extra()
    {
        ...
        $array_form = ['address' => ['label' => _("Address") . $this->required_field, 'html' => $address_html],
            'rest_api_resource' => [
    ...
    protected function listUserJira($filter)
    {
        $search = 'query=';
        if (isset($filter)) {
            $search .= urlencode((string) $filter);
        }
    ...
    protected function createTicketJira($ticket_arguments)
    {
        $argument = [
            'fields' => [
                'project' => ['id' => $ticket_arguments[$this->internal_arg_name[self::ARG_PROJECT]]],
