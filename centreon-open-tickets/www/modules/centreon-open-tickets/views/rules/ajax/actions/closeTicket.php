...
$query .= ") UNION ALL (";
$query .= "SELECT DISTINCT NULL as description, hosts.name as host_name, hosts.instance_id, mot.ticket_value, mot.timestamp FROM hosts, mod_open_tickets_link as motl, mod_open_tickets as mot";
$query .= " WHERE hosts.host_id IN (" . $hosts_selected_str . ")";
if (!$centreon_bg->is_admin) {
    $query .= " AND EXISTS(SELECT * FROM centreon_acl WHERE centreon_acl.group_id IN (" . $centreon_bg->grouplistStr . ") AND hosts.host_id = centreon_acl.host_id)";
}
$query .= " AND motl.host_id = hosts.host_id AND motl.service_id IS NULL AND motl.ticket_id = mot.ticket_id AND mot.timestamp > hosts.last_hard_state_change";
$query .= ") ORDER BY `host_name`, `description`, `timestamp` DESC";
...
<table class="table">
    ...
    <tr>
        <td class="FormRowField" style="padding-left:15px;">Tickets closed: ' . join(",", array_keys($removed_tickets)) . '.</td>
    </tr>';

if ($centreon_provider->doCloseTicket() && count($error_msg) > 0) {
    $resultat['msg'] .= '<tr>
        <td class="FormRowField" style="padding-left:15px; color: red">Issue to close tickets: ' . join("<br/>", $error_msg) . '.</td>
    </tr>';
}
$resultat['msg'] .= '</table>';
