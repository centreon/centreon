...
if (!empty($preferences['host_group']) && !empty($preferences["service"])) {
    $aclJoin = '';
    $aclSubRequest = '';
    $bindParams1 = [];
    $bindValuesAcl = [];
    ...
    $bindParams2 = [];
    [$bindValuesServices, $bindQueryServices] = createMultipleBindQuery(
        list: $serviceList,
        prefix: ':service_description_',
        bindType: PDO::PARAM_STR
    );
    $whereService2 = 'T1.description IN (' . $bindQueryServices . ')';

    // get host ids from data which was queried before
    $hostIds = array_column($data, "host_id");
    [$bindValuesHosts, $bindQueryHosts] = createMultipleBindQuery(
        list: $hostIds,
        prefix: ':host_id_',
        bindType: PDO::PARAM_INT
    );
    $whereService2 .= ' AND T1.host_id IN (' . $bindQueryHosts . ')';

    if ($accessGroups !== []) {
        $aclJoin = $centreon->user->admin == 0 ? ' INNER JOIN centreon_acl acl ON T1.service_id = acl.service_id' : '';
    }
    $bindParams2 = array_merge($bindValuesServices, $bindValuesHosts, $bindValuesAcl);

    /* ---------------------------
     * Query 2: Host Service Statuses
     * ---------------------------
     * Filter services by descriptions and hosts in hostgroups (use hosts from $data)
     */
