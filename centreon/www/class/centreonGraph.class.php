...
class CentreonGraph
{
    ...
    public function initCurveList()
    {
        ...
        if (isset($l_rselector)) {
            ...
            if (isset($this->metricsEnabled) && count($this->metricsEnabled) > 0) {
                $DBRESULT->bindValue(':l_rselector', $l_rselector);
            } else {
        ...
        if (isset($l_vselector)) {
            ...
            if (isset($this->metricsEnabled) && count($this->metricsEnabled) > 0) {
                $DBRESULT->bindValue(':l_vselector', $l_vselector);
            } else {
                $DBRESULT->bindValue(':index_id', $this->index);
            }
    ...
    private function getIndexData()
    {
        ...
        $DBRESULT->bindValue(':svc_instance', $svc_instance, PDO::PARAM_INT);
        $DBRESULT->execute();
        ...
        if ($DBRESULT->rowCount()) {
            ...
            if (preg_match("/meta_([0-9]*)/", $this->indexData["service_description"], $matches)) {
                ...
                $DBRESULT_meta->bindValue(':meta_id', $matches[1], PDO::PARAM_INT);
    ...
    {
        $res = $this->DBC->prepare('SELECT id FROM index_data WHERE host_id = :host_id AND service_id = :service_id');
        $res->bindValue(':host_id', $hostId, PDO::PARAM_INT);
        $res->bindValue(':service_id', $serviceId, PDO::PARAM_INT);
        $res->execute();
