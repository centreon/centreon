...
class CentreonACL
{
    ...
    public function checkHost($hostId)
    {
        ...
        return (bool) (in_array($hostId, $hostArray));
    }
    ...
    public function checkService($serviceId)
    {
        ...
        return (bool) (in_array($serviceId, $serviceArray));
    }
    ...
    public function getACLHostsTemporaryTableJoin($db, $fieldToJoin, $force = false)
    {
        ...
        return ' INNER JOIN ' . $tableName . ' ON ' . $tableName . '.host_id = ' . $fieldToJoin . ' ';
    }
    ...
    public function getACLServicesTemporaryTableJoin($db, $fieldToJoin, $force = false)
    {
        ...
        return ' INNER JOIN ' . $tableName . ' ON ' . $tableName . '.service_id = ' . $fieldToJoin . ' ';
    }
    ...
    public function getACLHostsTableJoin($db, $fieldToJoin, $force = false)
    {
        ...
        return ' INNER JOIN centreon_acl ' . $tempTableName . ' ON ' . $tempTableName . '.host_id = ' . $fieldToJoin
    ...
    public function getACLServicesTableJoin($db, $fieldToJoin, $force = false)
    {
        ...
        return ' INNER JOIN centreon_acl ' . $tempTableName . ' ON ' . $tempTableName . '.service_id = ' . $fieldToJoin
            . ' AND ' . $tempTableName . '.group_id IN (' . implode(',', $groupIds) . ') ';
    }
            ...
            if (is_array($groupIds) && count($groupIds)) {
                ...
                if (isset($data['type']) && $data['type'] == 'HOST'
                    ...
                    if ($data['action'] == 'ADD') {
                        ...
                        $hostACLStatement = CentreonDBInstance::getDbCentreonStorageInstance()->prepare(
                            $hostInsertACLQuery
                        );
                        ...
                        $serviceACLStatement = CentreonDBInstance::getDbCentreonStorageInstance()->prepare(
                            $serviceACLInsertQuery
                        );
    ...
    public function getServiceGroupAclConf($search = null, $broker = null, $options = null, $sg_empty = null)
    {
        ...
        if (is_null($options)) {
            $options = [
                'order' => ['LOWER(sg_name)'],
                'fields' => ['servicegroup.sg_id', 'servicegroup.sg_name'],
                'keys' => ['sg_id'],
                'keys_separator' => '',
                'get_row' => 'sg_name',
            ];
    ...
    public function getServiceServiceGroupAclConf($sgId, $broker = null, $options = null)
    {
        ...
        if (is_null($options)) {
            $options = [
                'order' => ['LOWER(host_name)', 'LOWER(service_description)'],
                'fields' => ['service.service_description', 'service.service_id', 'host.host_id', 'host.host_name'],
                'keys' => ['host_id', 'service_id'],
                'keys_separator' => '_',
            ];
        }
    ...
    public function getHostAclConf($search = null, $broker = null, $options = null, $host_empty = false)
    {
        ...
        if (is_null($options)) {
            $options = [
                'order' => ['LOWER(host.host_name)'],
                'fields' => ['host.host_id', 'host.host_name'],
                'keys' => ['host_id'],
                'keys_separator' => '',
                'get_row' => 'host_name',
            ];
    ...
    public function getHostServiceAclConf($host_id, $broker = null, $options = null)
    {
        ...
        if (is_null($options)) {
            $options = [
                'order' => ['LOWER(service_description)'],
                'fields' => ['s.service_id', 'service_description'],
                'keys' => ['service_id'],
                'keys_separator' => '',
    ...
    public function getPollerAclConf($options = [])
    {
        ...
        return $this->constructResult($sql, $options);
    }
    ...
    public static function duplicateHostAcl($hosts = []): void
    {
        ...
        foreach ($hosts as $copyId => $originalId) {
            ...
            CentreonDBInstance::getDbCentreonInstance()->query(
                sprintf($sql, $tbHostEx, $copyId, $tbHostEx, $originalId)
            );
        }
    }
    ...
    private function hasAccessToAllImageFolders(): bool
    {
        ...
        try {
            ...
            return false;
        } catch (CollectionException|ConnectionException $e) {
            throw new RepositoryException(
                message: 'Error while checking access to all image folders',
                context: ['accessGroups' => $accessGroups, 'bindQueryParameters' => $bindQueryParameters],
                previous: $e
            );
        }
    }
