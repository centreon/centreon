...
class CentreonDowntime
{
    ...
    public function getForEnabledHosts(): array
    {
        ...
        $request = <<<'SQL'
            ...
            INNER JOIN downtime dt ON dtp.dt_id = dt.dt_id
            INNER JOIN downtime_host_relation dtr ON dtp.dt_id = dtr.dt_id
            INNER JOIN host h ON dtr.host_host_id = h.host_id
    ...
    public function getForEnabledServices(): array
    {
        ...
        $request = <<<'SQL'
            ...
            FROM downtime_period dtp
            INNER JOIN downtime dt ON dtp.dt_id = dt.dt_id
            INNER JOIN downtime_service_relation dtr ON dtp.dt_id = dtr.dt_id
            INNER JOIN service s ON dtr.service_service_id = s.service_id
            INNER JOIN host_service_relation hsr ON hsr.service_service_id = s.service_id
            INNER JOIN host h ON hsr.host_host_id = h.host_id AND h.host_id = dtr.host_host_id
        ...
        UNION
            ...
            INNER JOIN downtime_service_relation dtr ON dtp.dt_id = dtr.dt_id
            INNER JOIN host h ON dtr.host_host_id = h.host_id
            INNER JOIN hostgroup_relation hgr ON hgr.hostgroup_hg_id = h.host_id
            INNER JOIN host_service_relation hsr ON hsr.hostgroup_hg_id = hgr.hostgroup_hg_id
            INNER JOIN service s ON s.service_id = hsr.service_service_id AND dtr.service_service_id = s.service_id
            WHERE h.host_activate = '1'
        SQL;
    ...
    public function getForEnabledHostgroups(): array
    {
        ...
        $request = <<<'SQL'
            ...
            FROM downtime_period dtp
            INNER JOIN downtime dt ON dtp.dt_id = dt.dt_id
            INNER JOIN downtime_hostgroup_relation dhr ON dtp.dt_id = dhr.dt_id
            INNER JOIN hostgroup_relation hgr ON dhr.hg_hg_id = hgr.hostgroup_hg_id
            INNER JOIN host h ON hgr.host_host_id = h.host_id
    ...
    public function getForEnabledServicegroups()
    {
        $request = <<<'SQL'
            SELECT dt.dt_id,
                   ...
                   s.service_register
            FROM downtime_period dtp
            JOIN downtime dt ON dtp.dt_id = dt.dt_id
            JOIN downtime_servicegroup_relation dtr ON dtp.dt_id = dtr.dt_id
            JOIN servicegroup_relation sgr ON dtr.sg_sg_id = sgr.servicegroup_sg_id
            JOIN service s ON sgr.service_service_id = s.service_id
            JOIN host h ON sgr.host_host_id = h.host_id
            JOIN servicegroup sg ON sgr.servicegroup_sg_id = sg.sg_id
            ...
            UNION DISTINCT
                ...
                JOIN downtime_servicegroup_relation dtr ON dtp.dt_id = dtr.dt_id
                JOIN servicegroup_relation sgr ON dtr.sg_sg_id = sgr.servicegroup_sg_id
                JOIN host_service_relation hsr ON sgr.hostgroup_hg_id = hsr.hostgroup_hg_id
                JOIN hostgroup_relation hgr ON hsr.hostgroup_hg_id = hgr.hostgroup_hg_id
                JOIN service s ON hsr.service_service_id = s.service_id
                JOIN host h ON hgr.host_host_id = h.host_id
