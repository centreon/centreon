...
$createAdditionalConnectorConfiguration = function(CentreonDB $pearDB) use(&$errorMessage): void {
    $errorMessage = 'Unable to create table additional_connector';
        $pearDB->query(
            <<<'SQL'
                CREATE TABLE IF NOT EXISTS `additional_connector` (
        ...
        <<<'SQL'
            CREATE TABLE IF NOT EXISTS `acc_poller_relation` (
                ...
                CONSTRAINT `acc_id_contraint`
                    FOREIGN KEY (`acc_id`)
                    REFERENCES `additional_connector` (`id`) ON DELETE CASCADE,
    ...
    if (false === (bool) $statement->fetch(\PDO::FETCH_COLUMN)) {
        $pearDB->query(
            <<<'SQL'
                ...
                VALUES ( 'Additional Connector', '/configuration/additional-connectors', '1', '1', 6, 618, 1, 1, '0')
                SQL
        );
    }
};
...
try {
    $createAdditionalConnectorConfiguration($pearDB);
