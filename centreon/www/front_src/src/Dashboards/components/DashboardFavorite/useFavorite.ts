...
import {
  ...
  labelDashboardUnmarkedAsFavorite
...
const addToFavorites = ({
  ...
  append(dashboardId, favoriteDashboards);
...
export const useFavorite = ({
  isFavorite,
  dashboardId
}: UseFavoriteProps): UseFavoriteState => {
  ...
  const { mutateAsync } = useMutationQuery({
    ...
    onError: () => {
      setProfile((currentProfile) => ({
        ...currentProfile,
        favoriteDashboards: getNewFavoriteList(
          isFavorite,
          Number(dashboardId),
  ...
  const toggleFavorite = useCallback(() => {
    mutateAsync({
      payload: {
        favorite_dashboards: getNewFavoriteList(
          isFavorite,
          Number(dashboardId),
          profile?.favoriteDashboards || []
        )
