...
describe.only("", () => {
	...
	it("ignores case sensitivity when searching static filters", () => {
		const getSearch = (criteriaValue) => `status_type:${criteriaValue}`;

		cy.findByPlaceholderText(labelSearch).type(
			`${getSearch("HARD")}{esc}{enter}`,
		);

		cy.findByPlaceholderText(labelSearch)
			.invoke("val")
			.should("equal", `${getSearch("hard")} `);

		cy.findByLabelText(labelSearchOptions).click();

		cy.findByText(labelShowMoreFilters).click();

		cy.findByTestId("status_types").find(`#${"Hard"}`).should("be.checked");
	});

	it.only("suggestoins", () => {
		cy.findByPlaceholderText(labelSearch).type("status_type:HA");
		cy.findByRole("menuitem", { name: "hard" });

		cy.makeSnapshot("");

		cy.findByPlaceholderText(labelSearch).clear();
		cy.findByPlaceholderText(labelSearch).type("status_type:HARD");
		cy.findByRole("menuitem", { name: "hard" }).should("not.exist");
		cy.findByRole("menuitem", { name: ",soft" }).should("exist");
		cy.makeSnapshot();
	});
});
