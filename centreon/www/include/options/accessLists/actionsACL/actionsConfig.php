...
switch ($o) {
    ...
    case ACL_ACTION_MASSIVE_ACTIVATION:
        ...
        if (isCSRFTokenValid()) {
            purgeCSRFToken();
            enableActionInDB(null, isset($select) ? $select : []);
        } else {
            unvalidFormMessage();
        }
        require_once(__DIR__ .  "/listsActionsAccess.php");
    ...
    case ACL_ACTION_MASSIVE_DEACTIVATION:
        ...
        if (isCSRFTokenValid()) {
            purgeCSRFToken();
            disableActionInDB(null, isset($select) ? $select : []);
        } else {
            unvalidFormMessage();
        }
        require_once(__DIR__ .  "/listsActionsAccess.php");
    ...
    case ACL_ACTION_DUPLICATION:
        ...
        if (isCSRFTokenValid()) {
            purgeCSRFToken();
            multipleActionInDB(isset($select) ? $select : [], $dupNbr);
        } else {
            unvalidFormMessage();
        }
        require_once(__DIR__ .  "/listsActionsAccess.php");
    ...
    case ACL_ACTION_DELETION:
        ...
        if (isCSRFTokenValid()) {
            purgeCSRFToken();
            deleteActionInDB(isset($select) ? $select : []);
        } else {
            unvalidFormMessage();
        }
        require_once(__DIR__ .  "/listsActionsAccess.php");
