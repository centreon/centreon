...
function getreportingTimePeriod()
{
    ...
    $reportingTimePeriod = [];
    $query = 'SELECT `cp_key`, `cp_value` FROM `contact_param` WHERE cp_contact_id IS NULL';
...
{
    global $pearDB;
    $stmt = $pearDB->prepare('SELECT `host_name` FROM `host` WHERE `host_id` = :id');
    $stmt->bindValue(':id', (int) $host_id, PDO::PARAM_INT);
    $stmt->execute();
    if ($row = $stmt->fetch()) {
...
{
    global $pearDB;
    $stmt = $pearDB->prepare('SELECT `hg_name` FROM `hostgroup` WHERE `hg_id` = :id');
    $stmt->bindValue(':id', (int) $hostgroup_id, PDO::PARAM_INT);
    $stmt->execute();
    if ($row = $stmt->fetch()) {
        return $row['hg_name'];
    }
...
{
    global $pearDB;
    $stmt = $pearDB->prepare('SELECT `service_description` FROM `service` WHERE `service_id` = :id');
    $stmt->bindValue(':id', (int) $service_id, PDO::PARAM_INT);
    $stmt->execute();
    if ($row = $stmt->fetch()) {
...
{
    global $pearDB;
    $stmt = $pearDB->prepare('SELECT `sg_name` FROM `servicegroup` WHERE `sg_id` = :id');
    $stmt->bindValue(':id', (int) $sg_id, PDO::PARAM_INT);
    $stmt->execute();
    if ($row = $stmt->fetch()) {
        return $row['sg_name'];
    }
