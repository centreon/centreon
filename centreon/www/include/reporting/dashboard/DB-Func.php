...
function getServicesLogs(array $services, $startDate, $endDate, $reportTimePeriod)
{
    ...
    if ($daysOfWeek !== []) {
        ...
        // No reporting days selected: return zero entries for each service
        // to preserve the same structure as when the query runs
        foreach ($services as $service) {
            $zeroStats = [];
            foreach (getServicesStatsValueName() as $name) {
                $zeroStats[$name] = 0;
            }
            $zeroStats['host_id'] = $service['hostId'];
            $zeroStats['service_id'] = $service['serviceId'];
            $servicesStats[$service['hostId']][$service['serviceId']] = $zeroStats;
        }
    }
