...
$helper->requireReadAccess(60410);
$pearDB = $helper->getDb();
...
// ACL filtering: a non-admin only sees dependencies built on the service groups it is granted.
if (! $helper->isAdmin()) {
    $acl    = $helper->getAcl();
    $aclIds = $acl !== null
        ? array_values(array_filter(array_map('intval', array_keys($acl->getServiceGroups()))))
        : [];
    ...
    foreach ($aclIds as $index => $aclId) {
        $placeholder       = 'acl_sg' . $index;
        $aclPlaceholders[] = ':' . $placeholder;
        $parameters[]      = QueryParameter::int($placeholder, $aclId);
    }

    $aclClause = ' AND rel.servicegroup_sg_id IN (' . implode(', ', $aclPlaceholders) . ')';
}

// A dependency shows up as soon as it references a granted service group on either side.
$conditions[] = <<<SQL
    (
        EXISTS (
            SELECT 1 FROM dependency_servicegroupParent_relation rel
            WHERE rel.dependency_dep_id = dep.dep_id{$aclClause}
        )
        OR EXISTS (
            SELECT 1 FROM dependency_servicegroupChild_relation rel
            WHERE rel.dependency_dep_id = dep.dep_id{$aclClause}
        )
    )
    SQL;
...
try {
    ...
    Logger::create(LogChannelEnum::WEB)->error(
        'AJAX listing: failed to fetch service group dependencies',
        ['exception' => $exception]
    );
