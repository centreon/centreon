...
// ACL filtering: a non-admin only sees dependencies whose parent hosts it is granted.
if (! $helper->isAdmin()) {
    $acl         = $helper->getAcl();
    ...
    $aclJoin = "INNER JOIN `{$aclDbName}`.centreon_acl acl "
        . 'ON acl.host_id = dhpr.host_host_id '
...
try {
    ...
    Logger::create(LogChannelEnum::WEB)->error(
        'AJAX listing: failed to fetch host dependencies',
        ['exception' => $exception]
    );
