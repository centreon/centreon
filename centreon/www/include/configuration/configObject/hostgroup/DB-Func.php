...
function testHostGroupExistence($name = null)
{
    ...
    return ! ($dbResult->rowCount() >= 1 && $hg['hg_id'] != $id);
}
            ...
            if ($hostGroupDatasetFilters === []) {
                ...
                foreach ($hostGroupDatasetFilters as $hostGroupDatasetFilter) {
                    $childDatasetFilter = array_filter(
                        $datasets,
                        fn (array $dataset) => $dataset['dataset_filter_parent_id'] === $hostGroupDatasetFilter['dataset_filter_id']
                    );

                    $parentDatasetFilter = array_filter(
                        $datasets,
                        fn (array $dataset) => $dataset['dataset_filter_id'] === $hostGroupDatasetFilter['dataset_filter_parent_id']
                    );
