...
{
    return (bool) (is_dir($path) && is_readable($path));
}

function is_executable_binary($path)
{
    return (bool) (is_file($path) && is_executable($path));
}

function is_writable_path($path)
{
    return (bool) (is_dir($path) && is_writable($path));
}

function is_writable_file($path)
{
    return (bool) (is_file($path) && is_writable($path));
}
...
function updateBackupConfigData($db, $form, $centreon)
{
    ...
    foreach ($radiobutton as $value) {
        $ret[$value] = isset($ret[$value], $ret[$value][$value])   && $ret[$value][$value] ? 1 : 0;
    }
...
function updateRemoteAccessCredentials($db, $form, $centreonEncryption): void
{
    ...
    unset($ret['submitC'], $ret['o'], $ret['centreon_token'], $ret['apiCredentials']);
