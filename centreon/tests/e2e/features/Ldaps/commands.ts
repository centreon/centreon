Cypress.Commands.add('addOrUpdateLdap', (body: Ldap) => {
  ...
  cy.getIframeBody().find('input[name="ar_name"]').clear().type(body.name);
  cy.getIframeBody()
  ...
  cy.fixture('../fixtures/users/user-with-access-to-allmodules.json').then(
    (user) => {
      cy.getIframeBody().find('input[name="bind_dn"]').clear().type(user.login);
      cy.getIframeBody()
  ...
  cy.getIframeBody().find('select[id="ldap_template"]').select('Posix');
  cy.getIframeBody()
