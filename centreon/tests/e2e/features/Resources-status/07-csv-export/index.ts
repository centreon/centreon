...
Given(
  'an admin user is logged in and redirected to the Resource Status page',
  () => {
    cy.loginByTypeOfUser({
      jsonName: 'admin',
      loginViaApi: false
...
Then(
  'the CSV file should contain the correct headers and the expected data',
  () => {
      ...
      cy.task('readCsvFile', { filePath }).then((csvContent) => {
        ...
        cy.fixture('resources/csvFIleWithAllPagesAndColumns.json').then(
          (expectedData) => {
            ...
            cy.log('Partial field comparison successful');
          }
        );
      });
    });
...
Then(
  'the CSV file should contain the updated headers and the expected data',
  () => {
    cy.task('getExportedFile', { downloadsFolder }).then((filePath) => {
      cy.task('readCsvFile', { filePath }).then((csvContent) => {
        const rows = (csvContent as string)
