...
Cypress.Commands.add(
  ...
  (body: ServiceGroupDependency) => {
    ...
    cy.getIframeBody()
      .find(`div[title="${body.dependent_service_groups[0]}"]`)
      .click();
    cy.getIframeBody()
      .find('input.btc.bt_success[name^="submit"]')
      .eq(0)
...
Cypress.Commands.add(
  ...
  (name: string, body: HostGroupService) => {
    ...
    cy.getIframeBody()
      ...
      .should('have.value', body.notinterval);
    cy.getIframeBody()
    ...
    cy.getIframeBody()
      ...
      .should('have.value', body.firstnotdelay);
    cy.getIframeBody()
      .find('input[name="service_recovery_notification_delay"]')
      .should('have.value', body.recoverynotdelay);
    //Relations
    cy.getIframeBody().contains('a', 'Relations').click();
    cy.get('body').click(0, 0);
    cy.getIframeBody()
