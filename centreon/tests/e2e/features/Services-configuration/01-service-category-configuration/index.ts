...
Then('a service category is configured', () => {
  ...
  cy.getIframeBody().find('input[name="sc_name"]').type('test');

  cy.getIframeBody()
    .find('input[name="sc_description"]')
    .type('test description');
  cy.getIframeBody().find('select#sc_svcTpl').next().click();
...
When('the user change the properties of a service category', () => {
  cy.waitForElementInIframe('#main-content', 'input[name="searchSC"]');
  cy.getIframeBody().contains('test').click();
...
Then('the properties are updated', () => {
  cy.waitForElementInIframe('#main-content', 'input[name="searchSC"]');
  cy.getIframeBody().contains('test description modified');
});
...
Then('the new service category has the same properties', () => {
  ...
  cy.get('iframe#main-content')
    ...
    .contains('Ping_1')
    .should('be.visible');
  cy.getIframeBody().contains('Ping_1').click();
  cy.waitForElementInIframe('#main-content', 'input[name="sc_description"]');
  cy.getIframeBody().find('input[value="Ping_1"]').should('exist');
  cy.getIframeBody().find('input[value="ping"]').should('exist');
...
Then('the deleted service category is not displayed in the list', () => {
  ...
  cy.getIframeBody().contains('test').should('not.exist');
});
