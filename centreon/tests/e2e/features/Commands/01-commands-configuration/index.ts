...
When('the user creates a command', () => {
  ...
  cy.getIframeBody().find('input[class="btc bt_success"][name^="submit"]').eq(0).click();
});
...
Then('the new command has the same properties', () => {
	cy.waitForElementInIframe('#main-content', `a:contains("${data.miscellaneous.name}_1")`);
...
When('the user deletes a command', () => {
  cy.visit('/centreon/main.php?p=60802&type=3');
  cy.waitForElementInIframe('#main-content', 'input[name="searchC"]');
  cy.getIframeBody().contains('a', `${data.miscellaneous.name}`).parents('tr').find('[alt="Delete"]').click();
});
