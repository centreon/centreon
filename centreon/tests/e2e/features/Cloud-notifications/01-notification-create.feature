...
Feature: Creating a Notification Rule
  ...
  @TEST_MON-38164
  Scenario Outline: Creating a <resource_type> Notification Rule for two contacts
    Given a '<resource_type>' with hosts and 'two contacts'
    When the user defines a name for the rule
    And the user selects a '<resource_type>' with associated events on which to notify
    And the user selects the 'two contacts'
    ...
    Then an email is sent to the configured 'two contacts' with the configured format

    Examples:
      | resource_type                           |
      | host group                              |
      | host group and services for these hosts |
