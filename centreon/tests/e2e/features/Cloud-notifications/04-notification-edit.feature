...
Feature: Editing notification rule configuration
  ...
  Scenario: Editing a notification rule resources configuration
    ...
    And the user saves and confirms
    Then only notifications for status changes of the updated resource parameters are sent once the notification refresh delay has been reached
  ...
  Scenario Outline: Editing a notification rule users configuration
    ...
    Then notifications for status changes are sent only to the updated <user_type> once the notification refresh delay has been reached
    Examples:
      | user_type      |
      | contact        |
      | contact groups |
