...
Then('the AJAX listing table is displayed with service rows grouped by host', () => {
  ...
  cy.getIframeBody().find('#clTableBody').contains('svc_test_host').should('exist');
});
...
Then('only the matching services are displayed', () => {
  cy.getIframeBody().find('#clTableBody').contains('svc_test_ping').should('exist');
  cy.getIframeBody().find('#clTableBody').contains('svc_test_cpu').should('not.exist');
});
...
Then('the toggle response is successful', () => {
  ...
  cy.get('@toggleSvc').its('response.body').should('have.property', 'success', true);
});
...
When('the service is disabled', () => {
  cy.executeSqlQuery({
    database: 'centreon',
    query: "UPDATE service SET service_activate = '0' WHERE service_description = 'svc_test_ping'"
  });
  visitAndWait(svcByHostPage);
});
...
Then('a duplicated service appears in the listing', () => {
  cy.waitForElementInIframe('#main-content', 'table.cl-listing-table');
  waitForAjaxRefresh();
  cy.getIframeBody().find('#clTableBody').contains('svc_test_ping_1').should('exist');
});
...
When('the user clicks on a service name', () => {
  cy.getIframeBody().find('#clTableBody').contains('a', 'svc_test_ping').click();
});

Then('the service edit form is displayed', () => {
  cy.waitForElementInIframe('#main-content', 'input[name="service_description"]');
  cy.getIframeBody().find('input[name="service_description"]').should('have.value', 'svc_test_ping');
});
...
Then('the search field still contains the search term', () => {
  cy.getIframeBody().find('#clSearchInput').should('have.value', 'svc_test_ping');
});
