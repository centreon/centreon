...
Feature: Create and update a new Agent Configuration without TLS
  ...
  Scenario: Create a CMA agent without TLS
    ...
    Then a warning message explaining the No TLS mode is displayed
  ...
  Scenario: Update a CMA agent without TLS
    ...
    And a warning message explaining the No TLS mode is shown
    When the user updates the CMA details
    And clicks "Save"
    Then the first configured agent is updated
  ...
  Scenario: Update a Telegraf agent without TLS
    ...
    And clicks "Save"
    Then the second configured agent is updated