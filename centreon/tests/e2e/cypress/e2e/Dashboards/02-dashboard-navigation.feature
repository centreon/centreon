...
Scenario: Get on the listing page when no dashboards are available  
  ...
  Then a special message and a button to create a new dashboard are displayed instead of the dashboards
...
Scenario: Select an immediately visible dashboard in a list of dashboards 
  ...
  Then they are redirected to the information page describing that dashboard
