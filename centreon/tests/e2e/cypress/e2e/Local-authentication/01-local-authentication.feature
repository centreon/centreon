...
Scenario: Enforcing password change policy
    ...
    When the administrator enables the password reuseability and sets and a user attempts to change its password multiple times in a row
...
Scenario: Editing the excluded users list
    ...
    Then the password expiration policy is not applyed anymore to the user added
