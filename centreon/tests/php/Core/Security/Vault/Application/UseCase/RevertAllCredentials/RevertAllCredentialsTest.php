...
use Core\Security\Vault\Application\UseCase\RevertAllCredentials\RevertAllCredentials;
use Core\Security\Vault\Application\UseCase\RevertAllCredentials\RevertAllCredentialsResponse;
...
beforeEach(function (): void {
    $this->useCase = new RevertAllCredentials(
        ...
        $this->accCredentialReverters = new \ArrayIterator([$this->createMock(VmWareV6CredentialReverter::class)]),
    );
});

it('should present an Error Response when no vault is configured', function (): void {
    $this->vaultEligibilityService
        ->expects($this->once())
        ->method('shouldUseVault')
        ->willReturn(false);
    $presenter = new RevertAllCredentialsPresenterStub();
...
it('should present a RevertAllCredentialsResponse when no error occurs', function (): void {
    $this->vaultEligibilityService
        ->method('shouldUseVault')
        ->willReturn(true);
    ...
    $presenter = new RevertAllCredentialsPresenterStub();
    ($this->useCase)($presenter);
    expect($presenter->response)->toBeInstanceOf(RevertAllCredentialsResponse::class)
        ->and($presenter->response->results)->toBeInstanceOf(CredentialReverter::class);
});
