...
final class ListHostGroupsChoicesProviderTest extends ApiTestCase
{
    private const BASE_ENDPOINT = '/api/configuration/hosts/host_groups';
    ...
    {
        $this->insertHostGroup("hg-A-{$this->tag}");
        $this->insertHostGroup("hg-B-{$this->tag}");
        ...
        self::assertSame(["hg-A-{$this->tag}", "hg-B-{$this->tag}"], array_column($member, 'name'));
        self::assertSame([], array_diff(array_keys($member[0]), ['@id', '@type', 'id', 'name']));
    }

    /**
     * Same ACL-scoping behavior as the generic /configuration/host_groups endpoint (unlike medias
     * or host_severities, host groups stay ACL-scoped for the host form too).
     */
    public function testItRestrictsListingToAccessibleHostGroupsForARestrictedUser(): void
    {
        $this->insertHostGroup("hg-A-{$this->tag}");
        $hostGroupBId = $this->insertHostGroup("hg-B-{$this->tag}");
        ...
        $response = $this->request('GET', self::BASE_ENDPOINT, ['query' => ['name' => ['lk' => $this->tag]]]);
        self::assertResponseIsSuccessful();
        self::assertSame(["hg-B-{$this->tag}"], array_column((array) $response->toArray()['member'], 'name'));
    }

    public function testItFiltersHostGroupsByNameUsingLikeOperator(): void
    {
        $this->insertHostGroup("match-{$this->tag}");
        $this->insertHostGroup("other-{$this->tag}");
    ...
    {
        $this->insertHostGroup("pg-{$this->tag}-A");
        $this->insertHostGroup("pg-{$this->tag}-B");
        $this->insertHostGroup("pg-{$this->tag}-C");
    ...
    public function testItRejectsAScalarNameFilter(): void
    {
        ...
        $this->request('GET', self::BASE_ENDPOINT, ['query' => ['name' => "hg-{$this->tag}"]]);
    ...
    private function restrictContactToHostGroups(int $contactId, array $hostGroupIds): void
    {
        $this->connection->insert('acl_resources', [
            'acl_res_name' => "hg-scope-{$this->tag}",
            'acl_res_alias' => "hg-scope-{$this->tag}",
            'acl_res_activate' => '1',
        ]);
