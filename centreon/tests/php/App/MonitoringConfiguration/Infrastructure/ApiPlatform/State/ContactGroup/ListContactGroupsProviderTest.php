...
final class ListContactGroupsProviderTest extends ApiTestCase
{
    ...
    public function testItFiltersByNameWithLikeOperatorMatch(): void
    {
        ...
        $name = $connection->fetchOne(
            "SELECT cg_name FROM contactgroup WHERE cg_name IS NOT NULL AND cg_name != '' LIMIT 1"
        );
        if ($name === false) {
    ...
    public function testItRejectsAZeroItemsPerPage(): void
    {
        $this->login();

        // itemsPerPage=0 must be a clean 400, not a 500 from the criteria's positive-integer assertion.
        $this->request('GET', self::BASE_ENDPOINT, ['query' => ['page' => '1', 'itemsPerPage' => '0']]);
        self::assertResponseStatusCodeSame(400);
    }
