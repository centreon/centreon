...
final class FakeHostGroupRepository implements HostGroupRepository
{
    /** @var array<int, HostGroup> */
    ...
    public function findNamesByIds(Collection $ids): Collection
    {
        $names = [];
        foreach ($ids as $id) {
            if (isset($this->hostGroups[$id->value])) {
                $names[$id->value] = $this->hostGroups[$id->value]->name;
            }
        }
    ...
    {
        return new Collection(array_values($this->hostGroups), HostGroup::class);
    }
}
