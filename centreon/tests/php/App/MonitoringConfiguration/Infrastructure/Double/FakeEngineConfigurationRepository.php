...
final class FakeEngineConfigurationRepository implements EngineConfigurationRepository
{
    /** @var array<int, EngineConfiguration> */
    public array $engineConfigurations = [];

    public function add(EngineConfiguration $engineConfiguration): void
    {
        do {
            $engineConfigurationId = mt_rand();
        } while (isset($this->engineConfigurations[$engineConfigurationId]));

        $reflection = new \ReflectionProperty(AggregateRoot::class, 'id');
        $reflection->setAccessible(true);
        $reflection->setValue($engineConfiguration, new EngineConfigurationId($engineConfigurationId));

        $this->engineConfigurations[$engineConfigurationId] = $engineConfiguration;
    }
}
