...
/**
 * @implements ActivityLogFactoryInterface<Poller>
 */
#[AsTaggedItem(index: Poller::class)]
final readonly class PollerActivityLogFactory implements ActivityLogFactoryInterface
{
    public function create(ActionEnum $action, AggregateRoot $aggregate, Actor $firedBy, \DateTimeImmutable $firedAt): ActivityLog
    {
        $target = new Target(
            ...
            type: TargetTypeEnum::Poller,
        );

        $details = [
            'ns_name' => $aggregate->name->value,
            'ns_ip_address' => $aggregate->address->value,
            'ns_activate' => $aggregate->isActivated ? '1' : '0',
            'poller_type' => $aggregate->pollerType->value,
        ];
