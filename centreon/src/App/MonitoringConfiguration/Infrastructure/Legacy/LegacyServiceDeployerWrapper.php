 ...
 * shared `Contact` is empty and `DeployServices` would refuse on its topology-role check, while
 * the action log reads its author from the legacy token storage rather than from that `Contact`.
 * Both are therefore impersonated, and undone in a `finally` so the impersonation lasts for the
 * call rather than for the container's lifetime.
 */
