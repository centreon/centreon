...
final class SharePlaylist
{
    ...
    public function __invoke(
        ...
        try {
            ...
            $this->validator->validateUniqueContacts($request->contacts);
            $this->validator->validateUniqueContactGroups($request->contactgroups);
            $this->validator->validatePlaylistExists($playlistId);
            if (! $this->rights->hasAdminRole()) {
                $this->validator->validateUserHasAccessToPlaylist($playlistId, $this->user);
            }
        } catch (\Throwable) {
