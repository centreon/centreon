...
class UserInformationFetcher implements AttributePathFetcherInterface
{
    ...
    public function fetch(string $accessToken, Configuration $configuration, Endpoint $endpoint): array
    {
        ...
        try {
            ...
            $exception = SSOAuthenticationException::requestForUserInformationFail();
            ...
            $this->loginLogger->error($scope, 'User Information Info: ', $content);
            $this->error(
    ...
    {
        return $endpoint->getType() === Endpoint::USER_INFORMATION;
    }
}
