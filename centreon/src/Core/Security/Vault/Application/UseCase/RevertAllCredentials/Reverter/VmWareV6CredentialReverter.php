...
/**
 * Reverse of {@see \Core\Security\Vault\Application\UseCase\MigrateAllCredentials\Migrator\VmWareV6CredentialMigrator}.
 *
 * @phpstan-import-type _VmWareV6Parameters from VmWareV6Parameters
 */
class VmWareV6CredentialReverter implements AccCredentialReverterInterface
{
    ...
    public function revertMigratedCredential(
        ...
        foreach ($parameters['vcenters'] as $index => $vcenter) {
            if ($credential->name === $vcenter['name'] . '_username') {
                $parameters['vcenters'][$index]['username'] = $plaintext;
            } elseif ($credential->name === $vcenter['name'] . '_password') {
                $parameters['vcenters'][$index]['password'] = $plaintext;
            }
        }
