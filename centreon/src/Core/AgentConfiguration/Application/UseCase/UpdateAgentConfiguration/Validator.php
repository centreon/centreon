...
class Validator
{
    ...
    ): void
    {
        ...
        // Check pollers are not already associated to an AC of same type.
        ...
        $unavailablePollers = $this->readAcRepository->findPollersByType($agentConfiguration->getType());
        ...
        if ([] !== $invalidPollers = array_intersect($unavailablePollerIds, $request->pollerIds)) {
            throw AgentConfigurationException::alreadyAssociatedPollers($agentConfiguration->getType(), $invalidPollers);
        }
    }

    // TODO delete ( + traversable classes) if nothing to validate.
