...
class Migration000022100000 extends AbstractCoreMigration implements LegacyMigrationInterface
{
    use LoggerTrait;
    private const VERSION = '22.10.0';
    ...
    public function up(): void
    {
         ...
         * @throws \Exception
         */
        $updatePollerMenusAcls = function (\CentreonDB $pearDB, int $topologyId): void
        {
            $stmt = $pearDB->prepare(
         ...
         * @throws \Exception
         */
        $updatePollerActionsAcls = function (\CentreonDB $pearDB, int $topologyId): void
        {
            ...
            foreach ($actionIdsToUpdate as $actionId) {
                 ...
                 */
                if (in_array($actionId, $actionIdsToExclude, true)) {
                    continue;
                }
         ...
         * @throws \Exception
         */
        $updatePollerAcls = function (\CentreonDB $pearDB) use ($updatePollerActionsAcls, $updatePollerMenusAcls): void
        {
            $stmt = $pearDB->query(
         ...
         */
        $linkFieldsToStreamType = function (\CentreonDB $pearDB, int $streamTypeId, array $fields): void
        {
         ...
         */
        $insertStreams = function (\CentreonDB $pearDB): array
        {
        ...
        /**
         * @return array{bbdo_server:array<string,string|int|null>,bbdo_client:array<string,string|int|null>}
         */
        $getFieldsDetails = function (): array
        {
            $bbdoServer = [
                [
                    'fieldname' => 'host',
         ...
         * @return int
         */
        $findListIdByFieldname = function (\CentreonDB $pearDB, string $fieldname): int
        {
            $stmt = $pearDB->prepare(
         ...
         */
        $insertGrpcListOptions = function (\CentreonDB $pearDB, int $listId): void
        {
         ...
         * @throws \Exception
         */
        $insertFieldOptions = function (\CentreonDB $pearDB, array $field) use ($findListIdByFieldname, $insertGrpcListOptions): void
        {
            if (in_array($field['fieldname'], ['encryption', 'compression', 'retention'], true)) {
         ...
         */
        $insertFields = function (\CentreonDB $pearDB, array $fields) use ($insertFieldOptions): array
        {
            ...
            foreach ($fields as &$field) {
                ...
                if (in_array($field['fieldtype'], ['radio', 'multiselect'], true)) {
         ...
         * @param \CentreonDB $pearDB
         */
        $createBbdoStreamConfigurationForms = function (\CentreonDB $pearDB) use ($insertStreams, $getFieldsDetails, $insertFields, $linkFieldsToStreamType): void
        {
        ...
        // Update-DB-22.10.0-beta.1.sql

        // Remove old columns in engine parameters table
        $pearDB->query(
            <<<'SQL'
                ALTER TABLE cfg_nagios
                DROP COLUMN `nagios_user`
                SQL
        );
        $pearDB->query(
            <<<'SQL'
                ALTER TABLE cfg_nagios
                DROP COLUMN `nagios_group`
                SQL
        );
        $pearDB->query(
            <<<'SQL'
