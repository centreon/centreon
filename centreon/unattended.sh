	...
	debian-release*)
		...
		11)
			...
			if [[ "$VENDORID" == "ARM" ]]; then
				error_and_exit "This version is not compatible for Debian on Raspberry."
			fi
			...
			for _repo in "${array_apt[@]}"; do
				...
				echo "deb https://packages.centreon.com/apt-plugins-$SIMPLEREPO/ $(lsb_release -sc) main" | tee /etc/apt/sources.list.d/centreon-plugins-$SIMPLEREPO.list
			done
