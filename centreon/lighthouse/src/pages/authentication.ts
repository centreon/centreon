...
export const generateReportForAuthenticationPage = async ({
  ...
  await flow.snapshot({ name: 'Authentication Snapshot', ...baseConfig });

  await page.waitForSelector('input[aria-label="Minimum password length"]');

  await flow.startTimespan({ name: 'Change letter case', ...baseConfig });
  await page.click('button[aria-label="Password must contain lower case"]');
  await flow.endTimespan();

  await page.click('button[aria-label="Password must contain lower case"]');

  await flow.startTimespan({ name: 'Change tab', ...baseConfig });
  ...
  await flow.startTimespan({
    name: 'Change OpenID connect activation',
    ...baseConfig,
  });
