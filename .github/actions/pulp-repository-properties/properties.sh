...
if [[ "$REPO_BASE" == "plugins" ]]; then
  ...
  if [[ "$DISTRIB_FAMILY" == "el" ]]; then
    ...
    # legacy alias suites (client compatibility): historical plugins names used
    # the bare "testing" segment; the stable suite name is unchanged (no alias)
    ...
    SUITE_ALIAS="$DISTRIB-$LEGACY_SEGMENT"
    [[ "$STABILITY" == "stable" ]] && SUITE_ALIAS=""
    TESTING_SUITE_ALIAS="$DISTRIB-$LEGACY_TESTING_SEGMENT"
    STABLE_SUITE_ALIAS=""
  ...
  if [[ "$DISTRIB_FAMILY" == "el" ]]; then
    ...
    # legacy alias suites (client compatibility with the pre-merge
    # stability-suffixed names, e.g. "trixie-26.10-stable")
    SUITE_ALIAS="$DISTRIB-$VERSION-$STABILITY_SEGMENT"
    TESTING_SUITE_ALIAS="$DISTRIB-$VERSION-$TESTING_SEGMENT"
    STABLE_SUITE_ALIAS="$DISTRIB-$VERSION-stable"
...

{
  ...
  echo "testing_suite_alias=$TESTING_SUITE_ALIAS"
  echo "stable_suite=$STABLE_SUITE"
  echo "stable_suite_alias=$STABLE_SUITE_ALIAS"
