...
repo=${ENV_CENTREON_REPO:-"unstable"}           #Default repository to used
...
wizard_autoplay=${ENV_WIZARD_AUTOPLAY:-"true"} #Default the install wizard is not run auto
...
function install_central() {
	...
	$PKG_MGR -q clean all --enablerepo="*" && $PKG_MGR -q install -y /tmp/*.rpm --enablerepo="$CENTREON_REPO"
	# $PKG_MGR -q clean all && $PKG_MGR install -y /tmp/*.rpm
