...
$query = "SELECT host_name, service_description, T2.service_id, T2.host_id, AVG(T3.current_value) as current_value, last_hard_state as status, T4.name, T5.hostgroup_id 
FROM services T1, index_data T2, metrics T3, hostgroups T4, hosts_hostgroups T5                                                                                                                             WHERE service_description LIKE '%".$preferences['service_description']."%' 
AND T3.index_id = id 
