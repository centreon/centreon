...
try {
  stage('Unit tests // RPMs packaging // Sonar analysis') {
    parallel 'unit tests backend centos7': {
      node {
        ...
        if (env.CHANGE_ID) { // pull request to comment with coding style issues
          ViolationsToGitHub([
            repositoryName: 'centreon-widget-ntopng-listing',
    ...
    'Debian bullseye packaging and signing': {
      node {
        dir('centreon-widget-ntopng-listing') {
          checkout scm
        }
        sh 'docker run -i --entrypoint "/src/centreon-widget-ntopng-listing/ci/scripts/widget-deb-package.sh" -w "/src" -v "$PWD:/src" -e "DISTRIB=bullseye" -e "VERSION=$VERSION" -e "RELEASE=$RELEASE" registry.centreon.com/centreon-debian11-dependencies:22.10'
  ...
  if ((env.BUILD == 'RELEASE') || (env.BUILD == 'REFERENCE')) {
    slackSend channel: "#monitoring-metrology",
        color: "#F30031",
        message: "*FAILURE*: `CENTREON WIDGET NTOPNG LISTING` <${env.BUILD_URL}|build #${env.BUILD_NUMBER}> on branch ${env.BRANCH_NAME}\n" +
            "*COMMIT*: <https://github.com/centreon/centreon-widget-ntopng-listing/commit/${source.COMMIT}|here> by ${source.COMMITTER}\n" +
            "*INFO*: ${e}"
  }
