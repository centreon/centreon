  ...
  stage('Unit tests // RPMs packaging // Sonar analysis') {
    ...
    'Debian bullseye packaging and signing': {
      node {
        ...
        sh 'docker run -i --entrypoint "/src/centreon-widget-ntopng-listing/ci/scripts/widget-deb-package.sh" -w "/src" -v "$PWD:/src" -e "DISTRIB=Debian11" -e "VERSION=$VERSION" -e "RELEASE=$RELEASE" registry.centreon.com/centreon-debian11-dependencies:22.10'
