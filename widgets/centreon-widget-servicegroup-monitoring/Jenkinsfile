...
stage('Source') {
  node {
    ...
    env.WIDGET = 'servicegroup-monitoring'
...
try {
    ...
    parallel 'centos7': {
      node {
        ...
        if (env.CHANGE_ID) { // pull request to comment with coding style issues
          ViolationsToGitHub([
            repositoryName: 'centreon-widget-servicegroup-monitoring',
        ...
        recordIssues(
          ...
          trendChartType: 'NONE',
          referenceJobName: 'centreon-widget-servicegroup-monitoring/master'
        )
  ...
  if ((env.BUILD == 'RELEASE') || (env.BUILD == 'REFERENCE')) {
    slackSend channel: "#monitoring-metrology",
        color: "#F30031",
        message: "*FAILURE*: `CENTREON WIDGET SERVICEGROUP MONITORING` <${env.BUILD_URL}|build #${env.BUILD_NUMBER}> on branch ${env.BRANCH_NAME}\n" +
            "*COMMIT*: <https://github.com/centreon/centreon-widget-servicegroup-monitoring/commit/${source.COMMIT}|here> by ${source.COMMITTER}\n" +
            "*INFO*: ${e}"
  }
