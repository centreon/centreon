...
stage('Source') {
  node {
    ...
    env.WIDGET = 'hostgroup-monitoring'
    ...
    parallel 'centos7': {
      node {
        ...
        if (env.CHANGE_ID) { // pull request to comment with coding style issues
          ViolationsToGitHub([
            repositoryName: 'centreon-widget-hostgroup-monitoring',
  ...
  if ((env.BUILD == 'RELEASE') || (env.BUILD == 'REFERENCE')) {
    slackSend channel: "#monitoring-metrology",
        ...
        message: "*FAILURE*: `CENTREON WIDGET HOSTGROUP MONITORING` <${env.BUILD_URL}|build #${env.BUILD_NUMBER}> on branch ${env.BRANCH_NAME}\n" +
            "*COMMIT*: <https://github.com/centreon/centreon-widget-hostgroup-monitoring/commit/${source.COMMIT}|here> by ${source.COMMITTER}\n" +
            "*INFO*: ${e}"
  }
