stage('Source') {
  node {
    ...
    sh './centreon-build/jobs/awie/18.9/mon-awie-source.sh'
...
try {
  stage('Unit tests') {
    parallel 'centos7': {
      ...
      node {
        sh 'setup_centreon_build.sh'
        sh './centreon-build/jobs/awie/18.9/mon-awie-unittest.sh centos7'
        step([
          $class: 'XUnitBuilder',
          thresholds: [
  ...
  stage('Package') {
    parallel 'centos7': {
      node {
        sh 'setup_centreon_build.sh'
        sh './centreon-build/jobs/awie/18.9/mon-awie-package.sh centos7'
      }
    }
    if ((currentBuild.result ?: 'SUCCESS') != 'SUCCESS') {
  ...
  stage('Bundle') {
    parallel 'centos7': {
      node {
        sh 'setup_centreon_build.sh'
        sh './centreon-build/jobs/awie/18.9/mon-awie-bundle.sh centos7'
      }
    }
    if ((currentBuild.result ?: 'SUCCESS') != 'SUCCESS') {
  ...
  stage('Acceptance tests') {
    parallel 'centos7': {
      node {
        sh 'setup_centreon_build.sh'
        sh './centreon-build/jobs/awie/18.9/mon-awie-acceptance.sh centos7'
        step([
          $class: 'XUnitBuilder',
          thresholds: [
            [$class: 'FailedThreshold', failureThreshold: '0'],
  ...
  if (env.BRANCH_NAME == 'master') {
    stage('Delivery') {
      node {
        sh 'setup_centreon_build.sh'
        sh './centreon-build/jobs/awie/18.9/mon-awie-delivery.sh'
      }
      if ((currentBuild.result ?: 'SUCCESS') != 'SUCCESS') {
