...
use vars qw(@pattern_output @action_list @macroList @statusList @idList @hostList $debug $DBType $DEBUG_ENABLED $USE_LONG_OPT);
use vars qw($alreadySent %macroCache @slotList);
...
my $pool_prefix = getPoolPrefix($hostname);
...
#############################################
## Try to find if the actual trap can be send
## Directly to a known slot (using alarm_id)
...
foreach my $filename (@fileList) {
	...
	close FILE;
	writeLogFile("CACHE: $idList[$t]", "DD");
	unlink($filename);
    }
    $t++;
}
...
if (@slotList ne 0) {
	...
	foreach my $time (@timeList) {
	    if (defined($macroCache{$hostname.";".$idList[$y]})) {
		my $slot_service = get_slot($hostname, $id, $dbh, $dbh2);
		if ($slot_service ne "nil") {
		    if (($status == 0 && $id ne "nil" && $FORCEFREE) || $status != 0) {
			send_command($host_name, $slot_service, $statusList[$y], $timeList[$y], $outputList[$y], $macroList[$y], $idList[$y], $dbh);
			$alreadySent = 1;
...
sub cleanEmptyMacros($$) {
	...
	while (my $data = $sth2->fetchrow_hashref()) {
		...
		while (my $d = $sth3->fetchrow_hashref()) {
			...
			updateMacro($d->{'hostname'}, $d->{'description'}, $poller->{'localhost'}, $d->{'macro'}, "empty", $timeRequest, $poller->{'id'});
		    }
		}
	    }
	}
