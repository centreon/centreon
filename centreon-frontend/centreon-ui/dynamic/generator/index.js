...
program
    .version('0.0.1')
    .option('-t, --topology [name]', 'Component topology name, if not defined, capitalized functional name will be used')
...
if (program.fname) {
    ...
    console.log(clc.blueBright.bgBlack("Run centreon-compgen --help for more info."));
}
...
async function replaceNames() {
    ...
    try {
        ...
        fs.renameSync(`${process.cwd()}/template/`, `${process.cwd()}/${program.fname}Compiler/`);
        fs.renameSync(`${process.cwd()}/${program.fname}Compiler/src/componentName`, `${process.cwd()}/${program.fname}Compiler/src/${program.fname}`);
        console.log(clc.green.bgBlack(`${program.topology} external component compiler generated.`));
        console.log(clc.white.bgBlack(`To use, replace COMPONENT_BUILD_PATH in package.json with the desired output folder.`));
        console.log(clc.white.bgBlack(`To run compiler, run "npm run install" and you can use "npm run compile"`));
    } catch (error) {
