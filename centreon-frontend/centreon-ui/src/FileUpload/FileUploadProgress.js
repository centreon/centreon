...
class FileUploadItem extends Component {
  render() {
    ...
    const cnFileUploadTitle = classnames(styles["file-upload-item-title"], titleStatus ? titleStatus : '');
    return (
      <React.Fragment>
        <div className={classnames(styles["file-upload-item"])}>
          ...
          <div className={classnames(styles.progress)}>
            <span className={classnames(styles["progress-bar"], styles[progressBar])}/>
          </div>
          <span className={classnames(styles["file-upload-item-info"], styles.percentage)}>
