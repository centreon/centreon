Feature:
  ...
  Scenario: Add comment to resources
    ...
    And I send a GET request to '/v21.10/monitoring/services?search={"$and":[{"host.name":"test"},{"service.description":"test_service_1"}]}'
    And I store response values in:
      | name      | path              |
      | hostId    | result[0].host.id |
      | serviceId | result[0].id      |

    When I send a POST request to '/v21.10/monitoring/resources/comments' with body:
    """
    {
      "resources": [
        {
