Feature:
  ...
  Scenario: Host timeline
    ...
    And I send a GET request to '/v21.10/monitoring/hosts?search={"host.name":"test"}'
    And I store response values in:
      | name   | path         |
      | hostId | result[0].id |
    And I send a POST request to '/v21.10/monitoring/hosts/<hostId>/check' with body:
    """
    {}
    """
    And I wait to get 1 result from "/v21.10/monitoring/hosts/<hostId>/timeline" (tries: 30)

    When I send a GET request to '/v21.10/monitoring/hosts/<hostId>/timeline?search={"type":"event"}'
  ...
  Scenario: Service timeline
    ...
    And I send a GET request to '/v21.10/monitoring/services?search={"$and":[{"host.name":"test"},{"service.description":"test_service1"}]}'
    And I store response values in:
      | name      | path              |
      | hostId    | result[0].host.id |
      | serviceId | result[0].id      |
    And I send a POST request to '/v21.10/monitoring/hosts/<hostId>/services/<serviceId>/check' with body:
    """
    {}
    """
    And I wait to get 1 result from "/v21.10/monitoring/hosts/<hostId>/services/<serviceId>/timeline" (tries: 30)

    When I send a GET request to '/v21.10/monitoring/hosts/<hostId>/services/<serviceId>/timeline?search={"type":"event"}'
